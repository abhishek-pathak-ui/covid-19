...
import './AyurvedicMedicine.css';

class AyurvedicMedicine extends React.Component {
  constructor(props) {
      super(props);
      this.state = {
      };
    ...
    onLoadcheck = () => {
        ...
        $('.slider').on('input change', function(){
                  $(this).next($(':text')).val(this.value);
        });
        $('.slider').next($(':text')).each(function(){
          var value = $(this).prev().attr('value');
          $(this).val(value);
        });


        $('.procedurecheckbox').click(function(){
          if ($('.procedurecheckbox:checked').length >= 2) {
            $(".procedurecheckbox").not(":checked").attr("disabled",true);
          }
          else
            $(".procedurecheckbox").not(":checked").removeAttr('disabled');
        });
        })
    }

    collectSelectedSymptoms = () => {
          var items=document.getElementsByClassName('symptomCheckbox');
          var selectedItems=[];
    ...
    collectPreExistingConsitions = () => {
          var items=document.getElementsByClassName('existingConsitionsCheckbox');
          var selectedItems=[];
          for(var i=0; i<items.length; i++){
            if(items[i].type=='checkbox' && items[i].checked==true)
              selectedItems.push(items[i].value);
    ...
    collectPreVitalList = () => {
          var items=document.getElementsByClassName('prevital');
          var selectedItems=[];
          var selectedNames=[];
          for(var i=0; i<items.length; i++){
            if(items[i].type=='text'){
              selectedItems.push(items[i].value);
              selectedNames.push(items[i].name);
            }
          }
          let preVitalList = {};
          for(let i=0;i<selectedNames.length;i++) {
            for(let j=0;j<selectedItems.length;++j){
              preVitalList[selectedNames[i]]=selectedItems[i];
            }
          }
          return preVitalList;
    }

    collectAvailableMedicines = () => {
          var items=document.getElementsByClassName('custom-select');
          var selectedItems=[];
          var selectedNames=[];
          for(var i=0; i<items.length; i++){
    ...
    collectProceduresCheckbox = () => {
          var items=document.getElementsByClassName('procedurecheckbox');
          var selectedItems=[];
          for(var i=0; i<items.length; i++){
            if(items[i].type=='checkbox' && items[i].checked==true)
              selectedItems.push(items[i].value);
    ...
    disableAllSelectedOptions = () => {
      var items = document.getElementsByClassName("messageSelectbox");
      for(var i=0; i<items.length; i++){
    ...
    dispatchOnChangeEvent = () => {
      var checkbox = document.createEvent("HTMLEvents");
      var items=document.getElementsByClassName('symptomCheckbox');
      for(var i=0; i<items.length; i++) {
        items[i].addEventListener('change', e => {
          if(e.target.checked){
              //do something
              const selectdId = e.target.id;
              var selectdIdSiblingList = $(`#${selectdId}`).siblings();
              document.getElementById(selectdIdSiblingList[1].id).disabled=false;
              document.getElementById(selectdIdSiblingList[1].id).selectedIndex= 1;
          } else {
            const selectdId = e.target.id;
            var selectdIdSiblingList = $(`#${selectdId}`).siblings();
            document.getElementById(selectdIdSiblingList[1].id).disabled=true;
            document.getElementById(selectdIdSiblingList[1].id).selectedIndex= 0;
          }
        });
      }
    ...
    dispatchOnChangeEventForPreExistingCon = () => {
      var checkbox = document.createEvent("HTMLEvents");
      var items=document.getElementsByClassName('existingConsitionsCheckbox');
      for(var i=0; i<items.length; i++) {
        items[i].addEventListener('change', e => {
    ...
    collectSelectedCheckBoxSymptoms = () => {
      //Reference the Table.
      var selectdOptionsValue = [];
      var items = document.getElementsByClassName("messageSelectbox");
        for(var i=0; i<items.length; i++){
          var strUser = items[i].options[items[i].selectedIndex].value;
          selectdOptionsValue.push(strUser);
        }
      var lowlist = [];
      var moderatelist = [];
      var highlist = [];
      for(var i=0; i<selectdOptionsValue.length; i++){
        if(selectdOptionsValue[i] === "Low") {
          lowlist.push(selectdOptionsValue[i]);
        } else if(selectdOptionsValue[i] ==  "Moderate") {
          moderatelist.push(selectdOptionsValue[i]);
        } else if (selectdOptionsValue[i] == "High") {
          highlist.push(selectdOptionsValue[i]);
        }
      }
      var values = (lowlist.length + (moderatelist.length*1.5) + (highlist.length*2));
      return values;
    }

    calculateHealthAndUpdateSelection = () => {
      var pointValue = this.collectSelectedCheckBoxSymptoms();
      document.getElementById("finalslider").value = 100 - pointValue;


      var xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          const responseOfML = JSON.parse(this.responseText).data;
          if(responseOfML) {
            $(".ml-data-container").show();
          }
          if (responseOfML.prescribed_meds) {
                    for (var i=0;i<responseOfML.prescribed_meds.length;i++) {
                        $("#panelBody").append('<li>' + responseOfML.prescribed_meds[i] + '</li>');
                    }
          }
          if (responseOfML.prescribed_procedures) {
                    for (var i=0;i<responseOfML.prescribed_procedures.length;i++) {
                        $("#panelBody1").append('<li>' + responseOfML.prescribed_procedures[i] + '</li>');
                    }
          }
        }
      };
      xhttp.open('GET', 'http://3.22.108.74:5000/get_details?email=amit_test@gmail.com', true);
    ...
    submitButton = () => {
      ...
      var selectdSymptomsList = this.collectSelectedSymptoms();
      var selectdPreExistingConsitions = this.collectPreExistingConsitions();
      var collectPreVitalData = this.collectPreVitalList();
      var availableMedicinesData = this.collectAvailableMedicines();
      var proceduresCheckboxData = this.collectProceduresCheckbox();
      var pointValue = this.collectSelectedCheckBoxSymptoms();
      document.getElementById("finalslider").value = 100 - pointValue;
      var preHealthVal = 100 - pointValue;
      var inputFormData = {
        user_id: "abhishekpathak919@gmail.com",
        symptoms: selectdSymptomsList,
        preconditions: selectdPreExistingConsitions,
        preexisting_meds: "Anti viral :Adefovir ,Chronic kidney disease :Demadex ,Joints Pain :Celebrex ,Shortness of breath:ipatropium bromide",
        previtals: collectPreVitalData,
        postvitals: {},
        prescribed_meds: availableMedicinesData,
        prescribed_procedures: proceduresCheckboxData,
        pre_health: preHealthVal,
        post_health: 87.9529
      }
      ...
      xhttp.open('POST', 'http://3.22.108.74:5000/submit_details', true);
      xhttp.setRequestHeader('Content-type', 'application/json');
      xhttp.send(obj);
    }
    ...
    componentDidMount() {
      this.onLoadcheck();
      this.collectSelectedSymptoms();
      this.disableAllSelectedOptions();
      this.dispatchOnChangeEvent();
      this.dispatchOnChangeEventForPreExistingCon();
    }
  ...
  <div className="HealthMeter" id="health">
      ...
      <input className="custom-range" type="range" min="1" max="100" value="100" id="finalslider" />
  </div>
...
<div className="main-wrapper">
  ...
  <div className="card">
    ...
    <button className="btn btn-success" onClick={this.calculateHealthAndUpdateSelection}>Submit</button>

    <div className="ml-data-container">
    <div className="panel panel-success">
      <div className="panel-heading"><h2>Prescribed Medicine</h2></div>
    ...
    <div className="panel panel-success">
      <div className="panel-heading"><h2>Prescribed Procedure</h2></div>
      <div className="panel-body">
      <ul id="panelBody1"></ul>
      </div>
    </div>
...
<center><button className="btn btn-success" onClick={this.submitButton} type="submit">Submit</button></center>
